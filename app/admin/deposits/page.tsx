...
import { AdminDeposit, DepositStatus } from "@/app/types/adminTypes/adminDeposit.types";
...
type DepositTab = "pending" | "approved" | "declined" | "all";

function matchesTab(deposit: AdminDeposit, tab: DepositTab) {
    switch (tab) {
        case "pending":
            return (
        ...
        case "approved":
            return deposit.status === DepositStatus.APPROVED;
        case "declined":
            return deposit.status === DepositStatus.DECLINED;
        default:
            return true;
    }
}
...
export default function AdminDepositsPage() {
    ...
    const [activeTab, setActiveTab] = useState<DepositTab>("pending");
    ...
    const filteredDeposits = useMemo(() => {
        ...
        return deposits
            .filter((deposit) => matchesTab(deposit, activeTab))
            .filter((deposit) => {
                if (!keyword) return true;

                return (
                    deposit.reference.toLowerCase().includes(keyword) ||
                    deposit.accountName.toLowerCase().includes(keyword) ||
                    deposit.bankName.toLowerCase().includes(keyword) ||
                    deposit.user.phone.toLowerCase().includes(keyword) ||
                    (deposit.user.email ?? "").toLowerCase().includes(keyword) ||
        ...
        () => ({
            ...
            pending: deposits.filter(
                (d) =>
                    ...
                    d.status === DepositStatus.UNDER_REVIEW,
            ).length,
            approved: deposits.filter(
                (d) => d.status === DepositStatus.APPROVED,
            ).length,
            declined: deposits.filter(
                (d) => d.status === DepositStatus.DECLINED,
            ).length,
        }),
        [deposits],
    );
